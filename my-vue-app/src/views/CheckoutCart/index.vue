<template>
  <div class="xtx-pay-checkout-page">
    <div class="container">
      <div class="wrapper">
        ...
        <!-- 提交订单 -->
        <div class="submit">
          <el-button type="primary" size="large" @click="createOrderBtn"
            >提交订单</el-button
          >
        </div>
      </div>
  ...
  <el-dialog v-model="showDialog" title="切换收货地址" width="30%" center>
    <div class="addressWrapper">
      <div
        class="text item"
        :class="{ active: changeAddressData.id === item.id }"
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="changeAddressBtn">确定</el-button>
      </span>
    </template>
  </el-dialog>
...
<script setup>
import { getCheckoutInfoAPI, createOrderApi } from "@/apis/checkoutCart";
...
const showDialog = ref(false); //切换地址弹框是否显示
const changeAddressData = ref({});
onMounted(() => getCheckoutCart());
...
const changeAddress = (item) => {
  console.log(item);
  changeAddressData.value = item;
};
//点击确认
const changeAddressBtn = () => {
  curAddress.value = changeAddressData.value;
...
const createOrderBtn = async () => {
  ...
  let res = await createOrderApi(data);
  console.log(res);
  //获取支付订单id
